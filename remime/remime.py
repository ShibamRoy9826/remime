...
[bold underline]Typical Usage:[/bold underline] [red]remime -\[mode] -\[options][/red]"""
    ...
    def error(self, message):
        if "unrecognized" in message:
            c.print(
                "[red]No such command found:( [/red] , try running [green]'remime -h'[/green]"
            )
        else:
...
class Remime(App):
    BINDINGS = [
        Binding(config["keybindings"]["quit_app"], "quit_app", "Quits The Application"),
        Binding(
            config["keybindings"]["toggle_dark"], "toggle_dark", "Toggles Dark Mode"
        ),
        Binding(config["keybindings"]["stop_ring"], "stop_ring", "To Mute The Alarm"),
        Binding(
            config["keybindings"]["pause_time"], "pause_time", "To Pause Alarm Time"
        ),
        Binding(
            config["keybindings"]["reset_time"], "reset_time", "To Reset Alarm Time"
        ),
    ]
    ...
    def __init__(
        ...
        if mode == "clock":
            ...
            self.target_label = strftime(f"%p , Target -> {time_list[0]:02,.0f}:{time_list[1]:02,.0f}:{time_list[2]:02,.0f}")
    ...
    def compose(self) -> ComposeResult:
        ...
        if self.mode == "pomodoro":
            if self.completed != 0:
                self.inputBox = Input(
                    value=self.begin[-1], type="text", id="task_input"
                )
            ...
            try:
                if self.mode == "pomodoro":
                    if config["pomodoro"]["automatic_mode_change"]:
                        ...
                        btn3.disabled = False
                        if (
                            self.wdg.pomodoro_count
                            == config["pomodoro"]["total_pomodoros"]
                        ):
...
def main():
    ...
    a.add_argument(
        "-at",
        "--at",
        help="Sets an alarm to ring in a specific time, requires -hr,-m, and -s parameters(Input in 24-hr clock)",
        action="store_true",
    )
    a.add_argument(
        "-p",
    ...
    if args.pomodoro:
        ...
        mode = "at"
        timeList=[int(args.hours),int(args.minutes),int(args.seconds)]
    elif args.clock_24:
        mode = "clock_24"
    elif args.stopwatch:
        mode = "stopwatch"
    ...
    if not is_notui:
        ...
        c.print(f"[blue bold]Remime:[/blue bold] [yellow]v{version}[/yellow]")
